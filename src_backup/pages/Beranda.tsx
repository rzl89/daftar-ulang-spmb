...
export default function Beranda() {
  ...
  return (
    <div className="w-full flex flex-col pb-20">
      {/* Hero Section */}
      <section className="relative w-full min-h-[90vh] flex flex-col items-center justify-center overflow-hidden bg-gradient-to-b from-primary to-primary-light text-white px-4 md:px-8">
        
        {/* Batik Kawung Background Motif — culturally relevant geometric pattern */}
        <div className="absolute inset-0 opacity-[0.07] pointer-events-none bg-batik-kawung" />
        ...
        >
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/90 text-primary border border-white/30 shadow-md mb-8">
          ...
          <div className="flex flex-wrap justify-center gap-4 mb-12">
            {Object.entries(timeLeft).map(([unit, value]) => (
              <div key={unit} className="flex flex-col items-center">
                <div className="w-20 h-20 md:w-24 md:h-24 bg-white/90 rounded-2xl flex items-center justify-center border-2 border-accent/20 shadow-lg mb-2">
                  <span className="text-3xl md:text-4xl font-bold font-heading text-primary">
                    {value.toString().padStart(2, '0')}
                  </span>
                </div>
                <span className="text-xs uppercase tracking-widest text-white/80 font-semibold">{unit}</span>
              </div>
            ))}
          </div>

          <Link to="/verifikasi">
            <Button size="xl" variant="accent" className="rounded-full animate-glow shadow-accent/50 hover:shadow-accent/80 text-lg px-8 group">
      ...
      {/* Steps Section — warm surface tone breaks up navy dominance */}
      <section className="py-20 px-4 md:px-8 w-full bg-surface-warm">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 relative">
          ...
          {steps.map((step, index) => (
            <motion.div 
              ...
              className="flex"
            >
              <Card className="flex-1 w-full border-0 shadow-lg shadow-slate-200/30 bg-white hover:-translate-y-2 transition-transform duration-300">
      ...
      {/* Map & Contact */}
      <section className="pb-20 pt-4 px-4 md:px-8 max-w-7xl mx-auto w-full">
